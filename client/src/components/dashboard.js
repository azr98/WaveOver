...
function Dashboard() {
  ...
  const fetchArguments = async (cognitoUserEmail) => {
    try {
      const response = await axios.get('http://10.0.0.8:5000/get_active_arguments', {
  ...
  const handleInitiate = async () => {
    try {
      if (userEmail) {
        ...
        const response = await axios.post('http://10.0.0.8:5000/submit_argument', argumentSubmitData);
